...
const markerCoordinates = marker =>
  JSON.parse(JSON.stringify(marker.getPosition()));
...
const Map = ({ center, activePan = 0, pans = [], setSurface }) => {
  ...
  const getNewPolygon = () =>
    new window.google.maps.Polygon({
      ..._POLYGON_DATA_,
      fillColor: colorPan,
      strokeColor: colorPan,
      editable: true,
    });

  const [polygons, setPolygons] = useState([getNewPolygon()]);

  useEffect(() => {
    const colors = pans.map(p => p.color);
    if (colors.length < polygons.length) {
      console.log(
        'Filtering based on colors ! polygons before=',
        polygons.length,
        ' and color : ',
        colors,
      );
      ...
      console.log(
        'pan being added should be handled in activePan useEffect already',
      );
  }, [pans]);

  const polygon = useMemo(() => {
    for (const p of polygons) p.setEditable(false);
  ...
  useEffect(() => {
    async function loadGoogleMaps() {
      ...
      const mapIni = new window.google.maps.Map(
      ...
      setMap(mapIni);
      setMarkers([]);
      if (polygon) polygon.setMap(mapIni);
    }
    loadGoogleMaps();
  }, [center]);

  useEffect(() => {
    if (!polygon || typeof polygon.setMap !== 'function') return;
    polygon.setMap(map);
  ...
  const handleClickPoints = useCallback(
    e =>
      setMarkers(markersIni => {
  ...
  const recalculateSurface = () => {
    ...
    if (path.length > 2) {
      const surface = window.google.maps.geometry?.spherical?.computeArea(
        path.map(p => ({
          lat: p.lat(),
          lng: p.lng(),
        })) /* .map(m => markerCoordinates(m)) */,
      );
